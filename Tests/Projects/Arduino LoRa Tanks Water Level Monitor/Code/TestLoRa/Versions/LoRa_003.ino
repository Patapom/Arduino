...
RECEIVE_RESULT WaitReceive( U16& _transmitterAddress, U8& _payloadLength, char*& _payload ) {
  int RSSI, SNR;  // Ignore those values
  return WaitReceive( _transmitterAddress, _payloadLength, _payload, RSSI, SNR );
}
RECEIVE_RESULT WaitReceive( U16& _transmitterAddress, U8& _payloadLength, char*& _payload, int& _RSSI, int& _SNR ) {
  char* response = WaitResponse();
  if ( response == NULL )
