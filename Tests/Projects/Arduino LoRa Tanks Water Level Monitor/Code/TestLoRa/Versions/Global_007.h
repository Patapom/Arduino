...
void  LogReply( char* _text ) { Log( "<OK> ", _text ); }
void  LogDebug( char* _text ) { Log( "<DEBUG> ", _text ); }
void  LogError( char* _text ) { Log( "<ERROR> ", _text ); }

void  Log() { Log( "" ); }
void  LogReply() { LogReply( "" ); }
void  LogDebug() { LogDebug( "" ); }
void  LogError() { LogError( "" ); }
...
public:
  str( char* _text, ... ) {
    ...
    if ( (ms_globalBuffer - s_globalBuffer) > 256 ) {
      // Fatal error!
      LogError( "str() buffer overrun! Fatal error!" );
      while ( true );
    }
    va_end( args );
  }
  ~str() {
    if ( m_string >= ms_globalBuffer ) {
      ...
      LogError( "~str() invalid string destruction order! Fatal error!" );
      while ( true );
    }
