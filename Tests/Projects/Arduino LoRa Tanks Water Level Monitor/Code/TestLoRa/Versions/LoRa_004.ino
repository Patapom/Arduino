...
RECEIVE_RESULT ReceiveWait( U16& _transmitterAddress, U8& _payloadLength, char*& _payload ) {
  int RSSI, SNR;  // Ignore those values
  return ReceiveWait( _transmitterAddress, _payloadLength, _payload, RSSI, SNR );
}
RECEIVE_RESULT ReceiveWait( U16& _transmitterAddress, U8& _payloadLength, char*& _payload, int& _RSSI, int& _SNR ) {
  char* response = WaitResponse();
  if ( response == NULL )
    return RR_TIMEOUT;  // Timeout

  return ExtractResponse( response, _transmitterAddress, _payloadLength, _payload, _RSSI, _SNR );
}
...
RECEIVE_RESULT ReceivePeek( U16& _transmitterAddress, U8& _payloadLength, char*& _payload ) {
  int RSSI, SNR;  // Ignore those values
  return ReceivePeek( _transmitterAddress, _payloadLength, _payload, RSSI, SNR );
}
RECEIVE_RESULT ReceivePeek( U16& _transmitterAddress, U8& _payloadLength, char*& _payload, int& _RSSI, int& _SNR ) {
  if ( LoRa.available() == 0 )
...
//  *p++ = '\0';  // Terminate string properly so it can be printed
  ...
  return ExtractResponse( LoRaBuffer, _transmitterAddress, _payloadLength, _payload, _RSSI, _SNR );
}

// Extracts the LoRa response in the form of "+RCV=<Address>,<Length>,<Data>,<RSSI>,<SNR>"
RECEIVE_RESULT  ExtractResponse( char* response, U16& _transmitterAddress, U8& _payloadLength, char*& _payload, int& _RSSI, int& _SNR ) {
  ...
  *payloadLengthEnd = '\0';
  _transmitterAddress = atoi( addressStart );
  _payloadLength = atoi( payloadLengthStart );
#ifdef DEBUG
  Serial.print( String( "Address = " ) + String( _transmitterAddress ) + String( ", payload size = " ) + String( _payloadLength ) );
#endif
