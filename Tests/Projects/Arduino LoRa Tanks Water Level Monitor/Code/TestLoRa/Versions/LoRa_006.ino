...
CONFIG_RESULT  SetWorkingMode( WORKING_MODE _workingMode, U16 _RXTime, U16 _sleepTime ) {
  if ( _workingMode == WM_SMART ) {
    if ( SendCommandAndWaitVerify( String( "AT+MODE=2," ) + _RXTime + "," + _sleepTime, "+OK" ) != RT_OK ) return CR_COMMAND_FAILED_AT_MODE;
  } else {
    if ( SendCommandAndWaitVerify( String( "AT+MODE=" ) + int(_workingMode), "+OK" ) != RT_OK ) return CR_COMMAND_FAILED_AT_MODE;
  }
  return CR_OK;
}
