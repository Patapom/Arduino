...
//  digitalWrite( PIN_LED_GREEN, runCounter & 1 );
  ...
  if ( cyclesCounter >= 15 * 60 ) {
    ...
    if ( ExecuteCommand_MeasureDistance() ) {
      Flash( PIN_LED_GREEN, 250, 1 );      // Notify of a new measurement
    } else {
      Flash( PIN_LED_RED, 150, 10 );      // Norify of a command failure!
    }
  }
  ...
  if ( command == "MEASURE" ) {
    ...
    if ( ExecuteCommand_MeasureDistance() ) {
      ...
      LogReply( str( "TIME=%d", bufferMeasurements[measurementsCounter & (MAX_MEASUREMENTS-1)].rawTime_microSeconds ) );
    } else {
      LogError( "Measure failed." );
    }

  } else if ( command.indexOf( "SETTIME=" ) == 0 ) {
    // Sets the reference time
    timeReference_seconds = atoi( command.c_str() + 8 );
    LogReply( str( "New reference time set to 0x%08X", timeReference_seconds ) );

  } else if ( command == "GETBUFFERSIZE" ) {
    // Return the amount of measurements in the buffer
    LogReply( str( "%d", measurementsCounter < MAX_MEASUREMENTS ? measurementsCounter : MAX_MEASUREMENTS ) );
    ...
    LogReply( str( "%d", count ) );

    U32 checksum = 0;
    for ( U8 i=0; i < count; i++, bufferIndex++ ) {
      Measurement&  m = bufferMeasurements[bufferIndex];
      LogReply( str( "%d;%d", m.timeStamp_seconds, m.rawTime_microSeconds ) );
    ...
    // Send final checksum
    LogReply( str( "CHECKSUM=%08X", checksum ) );
...
// Command Execution
//
bool  ExecuteCommand_MeasureDistance() {
...
// Fake reply...
reply = str( "DST0,%d,1234", (commandID-1) );

#else

  // Execute the command and wait for the reply
  char* reply = ExecuteAndWaitReply( 1, "DST0", commandID++, "" );
  if ( reply == NULL ) {
    // Command failed after several attempts!
LogDebug( "reply == NULL!" );
    measurementErrorsCounter++; // Count the amount of errors, after too many errors like this we'll consider an issue with the client module!
