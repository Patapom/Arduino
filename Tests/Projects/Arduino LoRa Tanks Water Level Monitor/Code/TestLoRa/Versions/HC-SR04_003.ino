...
// Returns the raw time in µs before we receive an echo (this is the total round trip back and forth Time Of Flight)
U32 MeasureEchoTime( U8 _pinTrigger, U8 _pinEcho ) {
  digitalWrite( _pinTrigger, HIGH );
  delayMicroseconds( 10 );
  digitalWrite( _pinTrigger, LOW );
  U32 duration_microSeconds = pulseIn( _pinEcho, HIGH );
...
float MeasureDistance( U8 _pinTrigger, U8 _pinEcho ) {
  ...
  return ConvertTimeOfFlightToDistance( time_microSeconds, 343.4f );  // Default speed of sound at 1 bar and 20°C in dry air is 343.4 m/s
}
