...
LogDebug( str( "Executing command (attempt #%d)", retriesCount ) );
...
LogDebug( "Invalid reply: bad format!" );
        ...
        U32 replyCommandID = strtol( reply + 8, ',', 16 );
        if ( replyCommandID != commandID-1 ) {
LogDebug( "Invalid reply: unexpected command ID!" );
          reply = NULL;
          continue;
        }
