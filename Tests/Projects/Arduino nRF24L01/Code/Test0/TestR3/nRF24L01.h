...
//    ClearMAX_RT();
    Write( NRF_STATUS, _BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT) );
...
//    ClearMAX_RT(); already cleared above, along with other flags...
    FlushTX();
...
//        status &= ~_BV(MAX_RT);
status = 0;
        status |= _BV(MAX_RT); // 8.5 Interrupts => The IRQ pin resets when MCU writes '1' to the IRQ source bit in the STATUS register
//Serial.println( str( "ClearMax_RT() => Status = 0x%02X (pre)", Status() ) );
    Write( NRF_STATUS, status );
//Serial.println( str( "ClearMax_RT() => Status = 0x%02X (post)", Status() ) );
  }
