...
//  Serial.println( str( "SPCR = 0x%02X", SPCR ) ); // 0x50
  ...
  // _enableAutoAcknowledge, indicates if the receiver should send an ACK packet back to the emitter (PTX and PRX must use the same _enableAutoAcknowledge values)
  void  PRX( bool _enableAutoAcknowledge ) {
    m_enableAutoAcknowledge = _enableAutoAcknowledge;

    // Configure as receiver
    U8  CRCBits = m_enableAutoAcknowledge ? m_CRCBits : 0;  // Disable CRC if auto-ACK is not used
    Write( NRF_CONFIG, CRCBits | _BV(PWR_UP) | _BV(PRIM_RX) ); // (0 + !MASK_RX_DR + !MASK_TX_DS + !MASK_MAX_RT | EN_CRC + CRCO = 2 bytes CRC + PWR_UP + PRIM_RX)

    Write( EN_AA, m_enableAutoAcknowledge ? 0x3F : 0x00 );       // Enable auto-ACK for all pipes (PTX must receive ACK after transmitting a packet)

    // This should only matter in PTX mode
    Write( SETUP_RETR, 0x00 );  // No automatic retransmission
  }
	
  // _enableAutoAcknowledge, indicates if the transmitter should expect an ACK packet back from the receiver and retransmit lost packets (PTX and PRX must use the same _enableAutoAcknowledge values)
  void  PTX( bool _enableAutoAcknowledge ) {
    m_enableAutoAcknowledge = _enableAutoAcknowledge;

    // Configure as transmitter
    U8  CRCBits = m_enableAutoAcknowledge ? m_CRCBits : 0;  // Disable CRC if auto-ACK is not used
    Write( NRF_CONFIG, CRCBits | _BV(PWR_UP) );

    if ( m_enableAutoAcknowledge ) {
      ...
      // !WARNING! At page 30 of the manual, we can find these lines indicating a minimum ARD of 500µs should be prefered:
...
//      Write( SETUP_RETR, 0x5F );  // Setup automatic retransmission ARD:4 = Wait 1500µs | ARC:4 = Retransmit 15 times
      Write( SETUP_RETR, 0x13 );  // Setup automatic retransmission ARD:4 = Wait 500µs | ARC:4 = Retransmit 3 times
    } else {
      Write( SETUP_RETR, 0x00 );  // No automatic retransmission
    }

    // This should only matter in PRX mode...
    Write( EN_AA, m_enableAutoAcknowledge ? 0x3F : 0x00 ); // Enable auto-ACK for all pipes
  }

  // Sets the air data rate and power level of the device
  void  SetAirDataRateAndPower( DATA_RATE _dataRate, POWER_LEVEL _power ) {
    U8  value = !_BV( PLL_LOCK ) | (_dataRate << 3) | (_power << 1) | _BV( LNA_HCURR ); // [06] RF_SETUP default = 0x01 => 000 + !PLL_LOCK = Don't force PLL signal (only use in test) | RF_DR:1 = 1Mbps Air data rate + RF_PWR:2 = -18dBm + LNA_HCURR:1
//Serial.println( str( "RF_SETUP = 0x%02X", value ) );
  ...
  void  SetCRCMode( CRC_MODE _CRC ) {
    switch ( _CRC ) {
      case CRC_DISABLED: m_CRCBits = 0; break;
      case CRC_1BYTE: m_CRCBits = _BV(EN_CRC); break;
      case CRC_2BYTES: m_CRCBits = _BV(EN_CRC) | _BV(CRCO); break;
    }
  }
...
//    ERROR( _pipe > 5, "SetReceiverPipeAddress", "Pipe index out of range" );
  ...
  // Sends a 32 bytes long  packet
  TRANSMIT_RESULT SendPacket( U8* _32BytesPayload ) {
    // Push a new packet
    if ( !WritePayload( _32BytesPayload ) )
      return TR_FIFO_FULL;

    return SendSingle();
  }

  // Sends a packet, payload must be 32 bytes max (dynamic payload length must be enabled)
  TRANSMIT_RESULT SendPacket( U8 _payloadLength, U8* _payload ) {
    // Push a new packet
    if ( !WritePayload( _payloadLength, _payload ) )
      return TR_FIFO_FULL;
...
// Reads a variable-length payload (dynamic payload length feature must be enabled)
...
//Serial.println( str( "Status %02X", m_status ) );
  ...
  // Pushes a new 32-bytes packet to the TX FIFO
  // Returns true if successful, false if FIFO is full
  bool  WritePayload( U8* _32BytesPayload ) {
    if ( IsTXFIFOFull() )
        return false;

    Select();

    m_status = SPI.transfer( m_enableAutoAcknowledge ? W_TX_PAYLOAD : W_TX_PAYLOAD_NO_ACK );

    U8  payloadLength = 32;
    while ( payloadLength-- ) {
      SPI.transfer( *_32BytesPayload++ );
    }
  ...
  // Pushes a new packet to the TX FIFO, payload must be 32 bytes max (dynamic payload length must be enabled)
  // Returns true if successful, false if FIFO is full
  bool  WritePayload( U8 _payloadLength, U8* _payload ) {
    if ( IsTXFIFOFull() )
        return false;
